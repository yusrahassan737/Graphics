...
def drawButton(scrn, btnX, btnY, text, textFormat, sceneChange, mx, my): 
    global iceFillClr, snow, iceLineClr, scene # globalize necessary variables
    ...
    if btnPos.collidepoint(mx, my): # if the mouse coordinates match up, change the text colour of the button
        ...
        pygame.draw.rect(scrn, iceFillClr, btnPos)
        scrn.blit(medText.render(text, 3, snow), (btnPos[0] + textFormat, btnPos[1] + 15, btnPos[2], btnPos[3]))

# Function to draw the menu
def menuScene(scrn, mx, my):
    # Initialize
    global scene, white, iceFillClr, iceLineClr, bkgdClr, medText, bigText, width, height, challenge, hitsToWin
    scene = 1
    scrn.fill(bkgdClr)
    ...
    for i in range(0, width, 100):
        for j in range(0, height, 100):
            scrn.blit(medText.render("*", 3, white), (i + 15, j + 15, 5, 5))
            scrn.blit(bigText.render("*", 3, white), (i + 70, j + 70, 5, 5))
            scrn.blit(bigText.render("-", 3, white), (i + 50, j + 30, 5, 5))
    
    # Title
    scrn.blit(bigText.render("Snowball Strike", 3, snow), (160, 80, 10, 10))
    
    # Challenge option
    challengeBtnRect = pygame.Rect(220, 160, 150, 40)
    # if the challenge button is pressed, switch state of challenge and change number of hits needed
    if challengeBtnRect.collidepoint(mx, my) and pygame.mouse.get_pressed()[0] == 1:
        if challenge:
            challenge = False
            hitsToWin = 15
        else:
            challenge = True
            hitsToWin = 25
    # Make the button change colour based on state of challenge        
    if challenge:
        pygame.draw.rect(scrn, iceLineClr, challengeBtnRect)
    else:
        pygame.draw.rect(scrn, iceFillClr, challengeBtnRect)
    # button text
    scrn.blit(medText.render("Challenge", 3, snow), (challengeBtnRect[0] + 25, challengeBtnRect[1] + 10, challengeBtnRect[2], challengeBtnRect[3])) 
    
    # Buttons
    drawButton(scrn, 220, 220, "How To Play", 15, 2, mx, my)
    drawButton(scrn, 220, 290, "Start!", 45, 3, mx, my)
...
def howToPlayScene(scrn, mx, my):
    ...
    # Draw a start button
    drawButton(scrn, 400, 30, "Menu", 45, 1, mx, my)

# Function to draw static parts of the game scene
def gameScene(scrn, iciclesY, armRise, flakePos, snowballX, snowballY, letter, numRows, mx, my):
    
    # Variables
    global scene, width, height, bkgdClr, iceFillClr, iceLineClr, white, snow, skin, darkBrown, green, darkGreen, mauve, snowballsClrs, snowballsPos, snowflakesPos, bigText, smallText, medText, words, validAlphaKeys, validNumKeys, countRowsleft, maxRows, hits, blocksChosen
    ...
    if countRowsleft and snowballLetter != "": # Conditions: Snowball has a letter, it isn't blocked by the icicles and countRowsleft is true
        
        maxRows -= numBlockRows # Update the number of rows left the user has
        countRowsleft = False # Stop updating until the next time all conditions are met 
    ...
    # Main snowball -display numBlockRows and letter chosen
    pygame.draw.circle(scrn, snowballsClrs[0], (snowballX, snowballY), 30)
    screen.blit(medText.render("Display " + str(numRows) + " row(s)", 3, skin), pygame.Rect(20, 320, 30, 30))
...
while gameRunning:
    ...
    while scene == 3 and gameSceneRunning:
        ...
        gameScene(screen, icicleYPos, armPos, snowflakesPos, snowballXPos, snowballYPos, snowballLetter, numBlockRows, mx, my)
        ...
        iciclesRect = (icicleXPos, icicleYPos - 110, 30, 50) # Rect object for icicle collision
        if snowballXPos >= iciclesRect[0] and snowballXPos <= iciclesRect[0] + iciclesRect[2] and snowballYPos >= iciclesRect[1] and snowballYPos <= iciclesRect[1] + iciclesRect[3]: # if the center of the circle fits inside the icicle collision rect, miss, crash, wait a bit and reset snowball
            screen.blit(bigText.render("MISS", 3, green), (snowballXPos - 40, snowballYPos + 20, 10, 10))
            crash = True
            pygame.display.flip()
        ...
        # Reset variables after a crash and check for win or loss
        if crash:
            snowballLetter = ""
            snowballXPos = 150
            snowballYPos = 190
            ...
            # Check for loss
            if hits < hitsToWin and maxRows == 0:
                ...
                # End status
                screen.blit(medText.render(str(hitsToWin - hits) + " hits left", 3, mauve), (250, 200, 10, 10))
        ...
        if runningCount % 7 == 0: # Every time the running count is a multiple of 10, snowflake positions are changed
            ...
            if hits >= hitsToWin:
                ...
                # End of game status
                screen.blit(medText.render(str(hits) + " hits from " + str(hitsToWin) + " and " + str(maxRows) + " row(s) left", 3, mauve), (150, 210, 10, 10))
